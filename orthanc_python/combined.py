...
def SaveDatasetDB(JSON, dataset):
    conn = get_PostGres_DB()  # Assumes get_PostGres_DB() returns a psycopg2 connection object
    if conn:
        AccessionNumber = JSON.get('AccessionNumber', "")
        StudyInstanceUID = JSON.get('StudyInstanceUID', "")

        ScheduledProcedureStepStartDate = ""
        AET = ""
        if 'ScheduledProcedureStepSequence' in JSON:
            step_sequence = JSON['ScheduledProcedureStepSequence'][0]
            ScheduledProcedureStepStartDate = step_sequence.get('ScheduledProcedureStepStartDate', "")
            AET = step_sequence.get('ScheduledStationAETitle', "")

        JSONinsert = json.dumps(JSON, sort_keys=True)
        datasetBytes = dataset_to_bytes(dataset)  # Convert dataset to bytes using your implementation

        try:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO mwl (completed, AccessionNumber, StudyInstanceUID, ScheduledProcedureStepStartDate, AET, MWLJSON, Dataset)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                    """,
                    (False, AccessionNumber, StudyInstanceUID, ScheduledProcedureStepStartDate, AET, JSONinsert, datasetBytes)
                )
                conn.commit()
                return True
        except psycopg2.Error as err:
